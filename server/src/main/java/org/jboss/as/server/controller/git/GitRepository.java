        ...
        if (gitDir.exists()) {
            ...
            try (Git git = Git.wrap(repository)) {
                ...
                if (!isLocalGitRepository(gitConfig.getRepository())) {
                    PullResult result = git.pull().setRemote(getRemoteName(gitConfig.getRepository())).setRemoteBranchName(branch).setStrategy(MergeStrategy.RESOLVE).call();
                    ...
                    if (!this.branch.equals(repository.getBranch())) {
                        CheckoutCommand checkout = git.checkout().setName(this.branch);
                        checkout.call();
